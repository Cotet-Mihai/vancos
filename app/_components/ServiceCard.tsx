...
export function ServiceCard({ service }: { service: Service }) {
  return (
    <div className="group flex flex-col gap-3 rounded-[1.75rem] border border-white/10 bg-surface p-6 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-lg">
      <h3 className="text-lg font-semibold text-paper">{service.title}</h3>
      <p className="text-sm leading-6 text-paper/70">{service.summary}</p>
      <Link
